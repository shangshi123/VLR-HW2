...
class DiffusionModel(nn.Module):
    def __init__(
        ...
        self.alphas_cumprod_prev = torch.cat((torch.zeros(1).to(self.device),self.alphas_cumprod[:-1]))
        ##################################################################
    ...
    def predict_denoised_at_prev_timestep(self, x, t: int):
        ...
        std = torch.ones(*x.size())
        z = torch.normal(mean,std).to(self.device)
    ...
    def ddim_step(self, batch, device, tau_i, tau_isub1, img, model_predictions, alphas_cumprod, eta):
        ...
        predicted_noise, x_0 = model_predictions(img,batched_tau_i)


        # Step 2: Extract \alpha_{\tau_{i - 1}} and \alpha_{\tau_{i}}
        alpha_cumprod = extract(alphas_cumprod,batched_tau_i,img.shape)
        alpha_sub1_cumprod = extract(alphas_cumprod,batched_tau_isub1,img.shape)
        beta = extract(self.betas,batched_tau_isub1,img.shape)
        # Step 3: Compute \sigma_{\tau_{i}}
        sigma_tau_i = (1.-alpha_sub1_cumprod)*beta/(1.-alpha_cumprod)
        log_variance_clipped = torch.log(sigma_tau_i.clamp(min =1e-20))
        # Step 4: Compute the coefficient of \epsilon_{\tau_{i}}
        eps_coef = torch.sqrt(1.-alpha_sub1_cumprod-sigma_tau_i)
        # print(eps_coef)
        # Step 5: Sample from q(x_{\tau_{i - 1}} | x_{\tau_t}, x_0)
        # HINT: Use the reparameterization trick
        mu = torch.sqrt(alpha_sub1_cumprod)*x_0 + eps_coef*predicted_noise
        mean = torch.zeros(*img.size())
        std = torch.ones(*img.size())
        z = torch.normal(mean,std).to(device)
        img = mu + torch.sqrt(sigma_tau_i)*z
        ##################################################################
        #                          END OF YOUR CODE                      #
        ##################################################################
